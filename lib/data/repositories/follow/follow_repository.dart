...
class FollowRepository extends GetxController {
  ...
  DocumentReference<Map<String, dynamic>> currentUserFollowingRef(String currentUserId, String targetUserId) {
    return _db.collection('users').doc(currentUserId).collection('following').doc(targetUserId);
  }

  DocumentReference<Map<String, dynamic>> targetUserFollowersRef(String currentUserId, String targetUserId) {
    return _db.collection('users').doc(targetUserId).collection('followers').doc(currentUserId);
  }

  Future<void> sendNotification(NotificationModel notification, String targetUserId) async {
    try {
      await _db.collection('users').doc(targetUserId).collection('notifications').doc().set(notification.toJson());
  ...
  Future<String?> followRequestExists(String fromUserId, String targetUserId) async {
    try {
      final query = await _db.collection('users').doc(targetUserId).collection('notifications')
        .where('FromUserId', isEqualTo: fromUserId)
        .where('Type', isEqualTo: 'followRequest').get();
  ...
  Future<void> deleteNotificationById(String notificationId, String targetUserId) async {
    try {
      await _db.collection('users').doc(targetUserId).collection('notifications').doc(notificationId).delete();
    } on FirebaseAuthException catch (e) {
      throw CbFirebaseAuthException(e.code).message;
    } on FirebaseException catch(e) {
      throw CbFirebaseException(e.code).message;
  ...
  Future<void> acceptFollowRequest(String notificationId, String targetUserId, String currentUserId) async {
    try {
      await startFollowingUser(targetUserId, currentUserId);

      await _db.collection('users').doc(currentUserId).collection('notifications').doc(notificationId).update({
  ...
  Future<void> stopFollowingUser(String currentUserId, String targetUserId) async {
    try {
      ...
      final acceptedQuery = await _db.collection('users').doc(currentUserId).collection('notifications').where('FromUserId', isEqualTo: targetUserId).where('Type', isEqualTo: 'followAccepted').get();
      batch.delete(acceptedQuery.docs.first.reference);
      
      final noticeQuery = await _db.collection('users').doc(targetUserId).collection('notifications').where('FromUserId', isEqualTo: currentUserId).where('Type', isEqualTo: 'followNotice').get();
  ...
  Future<void> startFollowingUser(String currentUserId, String targetUserId,) async {
    try { 
      final data = {
        'CreatedAt': FieldValue.serverTimestamp(),
      };
      ...
      final batch = _db.batch();
      batch.set(currentUserFollowingRef(currentUserId, targetUserId), data, SetOptions(merge: true));
      batch.set(targetUserFollowersRef(currentUserId, targetUserId), data, SetOptions(merge: true));
      await batch.commit();
    } on FirebaseAuthException catch (e) {
      throw CbFirebaseAuthException(e.code).message;
    } on FirebaseException catch(e) {
  ...
  Stream<List<NotificationModel>> loadNotifications(String userId) {
    try {
      return _db.collection('users').doc(userId).collection('notifications').orderBy('CreatedAt', descending: true).snapshots().map(
        (snapshot) {
